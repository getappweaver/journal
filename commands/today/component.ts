...
type RenderJournalTodayComponentProps = {
  alias: string;
    ...
    children: [
      {
        type: 'element',
        tag: 'menuItem',
        props: { label: 'Edit', action: editJournalEntryAction(alias, entry) },
      },
      {
        type: 'element',
        tag: 'menuItem',
...
function renderEntry(alias: string, entry: JournalEntry): WebNode {
  ...
  const headerChildren: WebNode[] = entry.title
    ? [
        textElement({
          value: entry.title,
          className: null,
...
export function renderJournalTodayComponent({
  alias,
  entries,
  recentEntries,
  editingEntry,
}: RenderJournalTodayComponentProps): WebNodeRoot {
